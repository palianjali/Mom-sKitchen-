...
const DailyCookingChallenge = () => {
  ...
  const toggleDetails = (id) => {
    setShowDetailsId((prevId) => {
      const newId = prevId === id ? null : id;
      setTimeout(() => {
        if (newId && refs.current[newId]) {
          refs.current[newId].scrollIntoView({ behavior: "smooth", block: "start" });
        }
      }, 100);
      return newId;
    });
    ...
    <div className="max-w-7xl mx-auto px-4 p-40">
      ...
      <div className="grid gap-9 sm:grid-cols-2 lg:grid-cols-3">
        {challenges.map((challenge) => (
          <div
            key={challenge.id}
            ref={(el) => (refs.current[challenge.id] = el)}
            className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-5 flex flex-col"
          >
            <img
              src={challenge.image}
            ...
            <div className="flex items-center gap-4 mb-3">
              ...
              <span className="flex items-center gap-1 text-blue-600">
                <MessageCircle size={18} /> {comments[challenge.id]?.length || 0}
              </span>
            </div>
